...
void disableRawMode() {
  if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &E.inital_termios) == -1)
    die("tcsetattr");
}

void enableRawMode() {
  if (tcgetattr(STDIN_FILENO, &E.inital_termios) == -1)
    die("tcgetattr");
...
void editorDrawStatusBar(struct abuf* ab) {
  ...
  int rlen = snprintf(status, sizeof(rstatus), "%d/%d", E.cy + 1, E.numrows);
  if (len > E.screenCols) len = E.screenCols;
